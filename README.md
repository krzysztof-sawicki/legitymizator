...
Program jest napisany na szybko, z mnóstwem "dirty-hacków". Testowane tylko pod Linuksem. Produkcyjnie sprawdzony przez [Instytut Edukacyjny AS](https://www.szkola.as). Możliwe, że zadziała pod Windows z niewielkimi zmianami (m.in. wskazanie pliku z czcionką w StudentID.py). 

## Wymagania
1. Python w wersji min. 3.10 z zainstalowanymi pakietami (sugerowane wersje, ale może działać ze starszymi):
- defusedxml==0.7.1
- fonttools==4.60.1
- fpdf2==2.8.4
- pdf2image==1.17.0
...
2.  W starszych wersjach wxPython (<4.2.3) konieczna jest niewielka poprawka w pliku wx/lib/pdfviewer/viewer.py. W pliku (około linijki 1080) należy zamienić treść `self.FitThisSizeToPage(wx.Size(width*sfac, height*sfac))` na `self.FitThisSizeToPage(wx.Size(int(width*sfac), int(height*sfac)))` zgodnie z [poprawką](https://github.com/wxWidgets/Phoenix/commit/0cf08c27fd6f9152c86879da042d8ca7e3af4f1d). Bez tej poprawki nie będzie działało wysyłanie przygotowanego pliku PDF do drukarki.
